...
def ai_agent_router():
    ...
    if "youtube" in cmd or "play" in cmd:
        ...
        q = q.strip()
        if not q:
            q = "Karuppu song" # Default fallback song
        vid = get_vid(q)
        if vid:
            target = f"https://www.youtube.com/embed/{vid}?autoplay=1&mute=0"
            msg = f"Playing {q}"
        else:
            target = "https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1"
            msg = "Playing video"
